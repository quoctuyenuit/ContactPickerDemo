...
- (void)layoutViews {
    ...
    [_searchBar.topAnchor constraintEqualToAnchor:self.view.topAnchor constant:UIApplication.sharedApplication.windows[0].safeAreaInsets.top].active = YES;
...
- (void)loadContact {
    ...
    [_viewModel loadContacts:^(BOOL isSuccess, NSError *error, NSUInteger numberOfContacts) {
        ...
        if (strongSelf) {
            if (isSuccess) {
                strongSelf->_contentViewController = [[ContactTableViewController alloc] initWithViewModel:self->_viewModel];

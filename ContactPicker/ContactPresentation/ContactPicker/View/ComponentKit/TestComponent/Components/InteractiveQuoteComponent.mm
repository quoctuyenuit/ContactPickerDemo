                     ...
                     context:(QuoteContext *)context
{
    ...
    [super newWithComponent:
     [CKFlexboxComponent newWithView:{ [UIView class], {CKComponentTapGestureAttribute(@selector(didTap)), {@selector(setBackgroundColor:), UIColor.redColor}} }
                                size:{}
                               style:{ .alignItems = CKFlexboxAlignItemsStretch, .justifyContent = CKFlexboxJustifyContentStart, .direction = CKFlexboxDirectionRow }
                            children:{
       
        {[CKOverlayLayoutComponent newWithComponent:[QuoteComponent newWithQuote:quote context:context]
                                            overlay:overlay]},
